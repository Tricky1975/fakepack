...
func pack(i []byte,)[]byte{
	...
	com,cer := xz.NewWriter(&buf) //, lzw.LSB, litw)
	if cer !=nil{
		fmt.Println("xz","write error",cer)
		panic("FATAL")
	}
	w, err := com.Write(i)
	if err != nil {
		fmt.Println("xz","write error:", err)
	}
	com.Close()
	if w==0{
...
func unpack(i []byte,size int)[]byte{
	...
	dec,cer := xz.NewReader(&buf) //, lzw.LSB, litw)
	if cer !=nil{
		fmt.Println("xz","read error",cer)
		panic("FATAL")
	}
	...
	if r!=size {
		fmt.Println("WARNING! XZ: Size of unpacked buffer doesn't match the requested size")
